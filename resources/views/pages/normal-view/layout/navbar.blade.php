    ...
    <div class="collapse navbar-collapse" id="navbarText">
        <ul class="navbar-nav mx-auto p-3">
            <li class="nav-item p-2">
                <a wire:navigate class="nav-link text-white text-center {{ '/' == request()->path() ? 'active2' : '' }}"
                    href="/"><i class="fa-light fa-house"></i> Home</a>
            </li>
            <li class="nav-item p-2">
                <a wire:navigate class="nav-link text-white text-center {{ 'about-us' == request()->path() ? 'active2' : '' }}"
                    href="/about-us"><i class="fa-light fa-question"></i> About Us</a>
            </li>
            <li class="nav-item p-2">
                <a wire:navigate class="nav-link text-white text-center {{ 'contact-us' == request()->path() ? 'active2' : '' }}"
                    href="/contact-us"><i class="fa-light fa-users"></i> Contact Us</a>
            </li>
            @if (auth()->check())
                <li class="nav-item p-2">
                    <a wire:navigate class="nav-link text-white text-center {{ 'products' == request()->path() ? 'active2' : '' }}"
                        href="/products"><i class="fa-light fa-box-open"></i> Products</a>
                </li>
                @role('user')
                    <li class="nav-item p-2">
                        <a wire:navigate class="nav-link text-white text-center {{ 'orders' == request()->path() ? 'active2' : '' }}"
                            href="/orders"><i class="fa-light fa-bag-shopping"></i> My Orders</a>
                    </li>
                    <li class="nav-item p-2">
                        <a wire:navigate class="nav-link text-white text-center {{ 'carts' == request()->path() ? 'active2' : '' }}"
                            href="/carts"><i class="fa-light fa-shopping-cart"></i> My Carts</a>
                    </li>
                    <li class="nav-item p-2">
                        <a wire:navigate class="nav-link text-white text-center {{ 'favorites' == request()->path() ? 'active2' : '' }}"
                            href="/favorites"><i class="fa-light fa-heart"></i> My Favorites</a>
                    </li>
                @endrole
            @else
                <li class="nav-item p-2">
                    <a wire:navigate class="nav-link text-white text-center {{ 'view-products' == request()->path() ? 'active2' : '' }}"
            ...
            <div class="dropdown-menu dropdown-menu-right" id="dropdown-setting" aria-labelledby="dropdownMenuButton">
                @if (auth()->check())
                    ...
                    @role('user')
                        ...
                        <div class="dropdown-divider"></div>
                    @endrole
                    <a wire:navigate class="nav-link p-3" href="#" data-toggle="modal" data-target="#logout"><i
