    ...
    <div class="card">
        <div class="card-body">
            <div class="col-sm-12">
                ...
                <label>Entries</label>
                <button class="btn btn-primary mb-3 me-2 float-end" data-bs-toggle="modal" wire:click="generateProductCode"
                    data-bs-target="#addProduct">
                    <i class="fa-solid fa-plus"></i> Add Product
                </button>
                <input type="search" class="form-control mb-3 mx-2 float-end" style="width: 198px;"
                    placeholder="Search" wire:model.live.debounce.200ms="search">
                        ...
                        @foreach ($products as $product)
                            <tr wire:key="{{ $product->id }}">
                                ...
                                @if ($product->productStocks() > 0)
                                    <td><span>{{ number_format($product->productStocks()) }} PC(s)</span></td>
                                @else
                                ...
                                @else
                                    <td>{{ $product->averageRatings() }} <i class="fa-solid fa-star"
                                            style="color: #ffd700;"></i>
                                    </td>
                                @endif
                                <td>&#8369;{{ number_format($product->product_price, 2, '.', ',') }} @if ($product->product_old_price !== null && $product->product_old_price !== $product->product_price)
                            ...
                            @endif --}}
                                ...
                                @if ((int) $product->orders_sum_order_quantity === 0)
                                    <td>No sold yet</td>
                                @else
                                    <td>x{{ $product->orders_sum_order_quantity }}</td>
                                @endif
                                <td>
                                    <div class="dropdown">
